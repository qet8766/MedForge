...
run_auth_contract_tests() {
  ...
  run_pytest_with_optional_parallel \
    ...
    tests/test_api.py::test_admin_list_users_pagination \
    tests/test_api.py::test_admin_list_sessions_filter_by_exposure \
...
run_config_and_log_contract_tests() {
  cd "${ROOT_DIR}/apps/api"
  ...
  run_pytest_with_optional_parallel \
    ...
    tests/test_config_data_isolation.py
}
...
run_remote_auth_smoke() {
  ...
  if [ "${signup_code}" = "409" ]; then
    ...
    # Body validation on 201 signup response
    assert_body_contains /tmp/phase2-remote-signup.out \
      "'data' in data and 'user' in data.get('data', {}) and 'id' in data['data']['user']" \
  ...
  # Body validation: verify /me response contains email field matching
  assert_body_contains /tmp/phase2-remote-me.out \
  ...
  echo "Remote auth smoke: signup/login, /me body validated, logout, post-logout 401"
  rm -f "${cookie_jar}"
}
...
main() {
  ...
  {
    echo "## Phase 2 Auth + API Contract Gate Evidence ($(date -u +%F))"
    echo ""
  ...
  if [ "${VALIDATE_PARALLEL}" = "1" ]; then
    run_parallel_checks \
      "Remote-External Auth Smoke (Body Validated)" "run_remote_auth_smoke" \
      "Auth and Session Contract Tests" "run_auth_contract_tests"
  else
    run_check_timed "Remote-External Auth Smoke (Body Validated)" "run_remote_auth_smoke"
    run_check_timed "Auth and Session Contract Tests" "run_auth_contract_tests"
  fi

  run_check_timed "Config and Log Contract Tests" "run_config_and_log_contract_tests"
